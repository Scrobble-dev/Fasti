...
forbidden='(^[[:space:]]*permissions:[[:space:]]*write-all([[:space:]]|$)|^[[:space:]]*[A-Za-z0-9_-]+:[[:space:]]*write([[:space:]]|$)|docker/login-action|release-action|upload-release-action|action-gh-release|attest-build-provenance|push-to-registry:[[:space:]]*true|push:[[:space:]]*true|(^|[[:space:]])git[[:space:]]+push([[:space:]]|$)|(^|[[:space:]])gh[[:space:]]+pr[[:space:]]+(merge|ready|close|edit)([[:space:]]|$)|(^|[[:space:]])(docker|podman|nerdctl|oras)[[:space:]]+(image[[:space:]]+|manifest[[:space:]]+)?push([[:space:]]|$)|(^|[[:space:]])skopeo[[:space:]]+(copy|sync)[^#]*docker://|docker[[:space:]]+buildx[[:space:]]+build.*--push([=[:space:]]|$)|^[[:space:]]*--push([=[:space:]]|$)|(npm|pnpm|yarn|cargo)[^[:alnum:]_#\n]+publish([^[:alnum:]_]|$)|(^|[[:space:]])(npm|pnpm|yarn|bun)[[:space:]]+run[[:space:]]+publish([[:space:]]|$)|(^|[[:space:]])gh[[:space:]]+release([[:space:]]|$)|(^|[[:space:]])gh[[:space:]]+api[^#\n]*/releases([[:space:]]|$))'
failed=0
