...
forbidden='(^[[:space:]]*[A-Za-z0-9_-]+:[[:space:]]*write([[:space:]]|$)|docker/login-action|release-action|upload-release-action|action-gh-release|attest-build-provenance|push-to-registry:[[:space:]]*true|push:[[:space:]]*true|(^|[[:space:]])(docker|podman|nerdctl|oras)[[:space:]]+push([[:space:]]|$)|(^|[[:space:]])(npm|pnpm|yarn|cargo)[[:space:]]+publish([[:space:]]|$)|(^|[[:space:]])gh[[:space:]]+release([[:space:]]|$)|(^|[[:space:]])gh[[:space:]]+api[^#\n]*/releases([[:space:]]|$))'
failed=0
