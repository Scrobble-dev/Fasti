...
The qualifying CI path evaluates one canonical idle run per architecture. It recomputes CPU from the 900 raw cgroup observations and checks the governed OCI and contract-pack sizes. The unpacked OCI measurement is the sum of the ordered uncompressed layers referenced by the retained Docker-save manifest. The verifier accepts the legacy Docker archive or Docker's containerd-backed OCI layout. It binds the immutable image ID to either one exact root descriptor or one selected manifest's exact config digest, cross-checks that selection against `manifest.json`, verifies every blob digest and descriptor size, and hashes the uncompressed layers against the ordered image `diff_ids`. It also checks the config architecture and operating system, and derives the contracts tree, Dockerfile digest, and exact Git-archive digest from the bound commit before accepting the image labels. The native installed-runtime and native archive budgets remain `not_applicable` until B8 produces those distributions. The optional comparison harness below continues to use at least five independent native and OCI runs. An omitted or hand-entered result cannot close B1.
...
The checked-in Raspberry Pi image allowlist pins the official `2026-06-18-raspios-trixie-arm64-lite.img.xz` filename and SHA-256 published by Raspberry Pi. Pi preflight hashes the retained compressed image bytes and requires both that exact filename and digest; `/etc/os-release` proves only the running release fields and cannot turn an arbitrary image into Raspberry Pi OS Lite. J4125 records its retained image digest without making an edition claim.
