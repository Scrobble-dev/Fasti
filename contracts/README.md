...
The production OpenAPI document contains health, durable node initialization and first-client enrollment, observation acceptance, and Record, identifier, and namespace operations. `fastid` mounts the durable routes only for a loopback bind with an explicit `FASTI_DATA_ROOT`, including an explicit loopback-only container forward. The separate B1 conformance OpenAPI and router are compiled only with `conformance-fixture`; its server binds only to IPv4 loopback, keeps bounded state in memory, and declares fixture-only, no-durability success. The SSE receipt channel belongs to AsyncAPI rather than being placed in the finite OpenAPI document. Fixture behavior does not authorize other B2 runtime behavior.
...
Provider seeds and manifest examples remain future adapter inputs, not working integrations. Durable setup does not make identity review or portability paths public. The local browser Workbench consumes the generated SDK for health and Record reads. It adds no contract surface. There is no supported install, release, deployed web interface, desktop package, or player.
