...
| First Fasti administrator                                                                           | Trusted local Unix `fasti access bootstrap-administrator` ceremony while `fastid` is stopped; TrailBase owns each person's password and the CLI returns no active session |
| Fasti browser sessions                                                                              | Merged ordinary-browser C1 implements fixed-origin exchange, issuance, inventory, rotation, profile selection, and revocation; packaged Tauri authentication remains deferred   |
...
- **C1: Local human access** — direct TrailBase exchange, opaque Fasti sessions, A+C UI, and the ordinary-browser delivery gates merged in [PR #119](https://github.com/Scrobble-dev/Fasti/pull/119). Packaged Tauri authentication and its platform evidence are deferred to `C1-TAURI-AUTH` and are not claimed.
