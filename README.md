...
- **C1: Local human access** — direct TrailBase exchange, opaque Fasti sessions, A+C UI, and the in-scope delivery runner are implemented locally; review, exact-head, merge, and merged-tree evidence remain open. Packaged Tauri authentication and its platform evidence are deferred to `C1-TAURI-AUTH` and are not claimed.
