...
use openidconnect::{
    AccessToken, AccessTokenHash, Audience, AuthType, AuthorizationCode, ClientId, ClientSecret,
    CsrfToken, EmptyAdditionalClaims, EndSessionUrl, HttpRequest, IssuerUrl, JsonWebKeyId,
    LogoutRequest, Nonce, OAuth2TokenResponse, PkceCodeChallenge, PkceCodeVerifier,
    PrivateSigningKey, RedirectUrl, RequestTokenError, StandardClaims, SubjectIdentifier,
    TokenResponse,
};
...
#[test]
fn confidential_client_authentication_preserves_wire_binding_and_error_boundaries() {
    // Public synthetic credentials. Expected encodings are literal RFC 6749 form-before-Basic
    // bytes, not values generated by the same serializer being qualified.
    const CLIENT_ID: &str = "synthetic: café+&=";
    const SECRET: &str = "synthetic:päss +&=";
    const ENCODED_SECRET: &str = "synthetic%3Ap%C3%A4ss+%2B%26%3D";
    const PKCE: &str = "dBjftJeZ4CVP-mB92K27uhbUJU1p1r_wW1gFWFOEjXk";
    const BASIC: &str = "Basic c3ludGhldGljJTNBK2NhZiVDMyVBOSUyQiUyNiUzRDpzeW50aGV0aWMlM0FwJUMzJUE0c3MrJTJCJTI2JTNE";

    for auth_type in [AuthType::BasicAuth, AuthType::RequestBody] {
        let in_body = matches!(auth_type, AuthType::RequestBody);
        let client = CoreClient::from_provider_metadata(
            serde_json::from_value::<CoreProviderMetadata>(metadata()).unwrap(),
        ...
        .set_redirect_uri(RedirectUrl::new("https://fasti.example.test/callback".into()).unwrap());
        for outcome in ["success", "provider-error", "lost-response"] {
            let count = RefCell::new(0);
            let http = |request: HttpRequest| {
                *count.borrow_mut() += 1;
                assert_eq!(request.method(), "POST");
                ...
                if !in_body {
                    ...
                    assert!(
                        !header.is_sensitive(),
                        "pinned Basic-header behavior changed"
                    );
                }
                let mut expected = format!(
                    "grant_type=authorization_code&code=synthetic-code&code_verifier={PKCE}"
                );
                if in_body {
                    expected.push_str("&client_id=synthetic%3A+caf%C3%A9%2B%26%3D&client_secret=");
                    expected.push_str(ENCODED_SECRET);
                }
                expected.push_str("&redirect_uri=https%3A%2F%2Ffasti.example.test%2Fcallback");
                assert_eq!(request.body(), expected.as_bytes());
                let pairs: Vec<_> =
                    openidconnect::url::form_urlencoded::parse(request.body()).collect();
                assert_eq!(pairs.len(), if in_body { 6 } else { 4 });
                for field in ["client_id", "client_secret"] {
                    assert_eq!(
                        pairs.iter().filter(|(name, _)| name == field).count(),
                        usize::from(in_body)
                    );
                }
                if outcome == "lost-response" {
                    return std::future::ready(Err::<openidconnect::HttpResponse, _>(
                        io::Error::new(
                            io::ErrorKind::ConnectionReset,
                            "synthetic lost confidential-client response",
                        ),
                    ));
                }
                let (status, body) = if outcome == "provider-error" {
                    (
                        400,
                        json!({"error":"invalid_client", "error_description": ENCODED_SECRET}),
                    )
                } else {
                    (
                        200,
                        json!({"access_token":"synthetic-confidential-access", "token_type":"Bearer"}),
                    )
                };
                std::future::ready(Ok(openidconnect::http::Response::builder()
                    .status(status)
                    .header("content-type", "application/json")
            ...
            match (outcome, result) {
                ("success", Ok(response)) => {
                    assert_eq!(
                        ...
                        "synthetic-confidential-access"
                    );
                    assert!(response.id_token().is_none());
                }
                ("provider-error", Err(RequestTokenError::ServerResponse(error))) => {
                    assert_eq!(error.error().to_string(), "invalid_client");
                    assert_eq!(
                        error.error_description().map(String::as_str),
                        Some(ENCODED_SECRET)
                    );
                    // Inspect only public fixture text: vendor Display retains provider echoes.
                    assert!(error.to_string().contains(ENCODED_SECRET));
                }
                ("lost-response", Err(RequestTokenError::Request(error))) => {
                    assert_eq!(error.kind(), io::ErrorKind::ConnectionReset);
                }
                _ => panic!("unexpected confidential-client qualification outcome"),
            }
        }
    }
    // Credential placement is not a governed transport, ID-token validation or erasure proof.
}
