...
- For the isolated [C3 source-notice custody tool](qualification/access-c3-native-attribution/README.md), run its 14 standard-library tests and documented exact-archive build/verify checks. Root Cargo tests do not cover this Python-only package. Preserve the frozen source selection and original inventory; no extraction, native execution, production attribution or crypto-profile approval is implied. Follow the [bounded delivery gate](docs/plans/fasti-access-c3-native-attribution.md).
...
- For the isolated [C3 framing qualification](qualification/access-c3-framing/README.md), follow its target and native-override isolation steps, then run `cargo +1.97.1 test --offline --locked --manifest-path qualification/access-c3-framing/Cargo.toml -j 2 -- --test-threads=1` and the documented release, formatting, Clippy and advisory checks. Each full debug/release suite must pass 22 unit tests, including constructor prefix-write failure, sink release and terminal rejection after final physical-EOF read failure, plus two compile-fail doctests that reject Debug formatting and Clone. Root workspace tests exclude this package; it does not activate production encryption, joint backups or recovery, or prove physical secret erasure. Follow the [constructor regression gate](docs/plans/fasti-access-c3-constructor-failures.md) and [final-EOF regression gate](docs/plans/fasti-access-c3-final-eof.md) for those bounded additions.
