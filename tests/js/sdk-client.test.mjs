...
test("connection endpoints preserve custom domains and expose loopback alternatives", () => {
  ...
  assert.deepEqual(connectionEndpoint("https://fasti.internal:9443", "build"), {
    url: "https://fasti.internal:9443",
    source: "build",
    managed: true,
    trust: "https",
    loopbackAliases: [],
...
test("connection endpoints reject values that are not safe origins", () => {
  for (const value of [
    "ftp://fasti.internal",
