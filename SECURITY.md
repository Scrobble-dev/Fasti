...
- C1 implements human-account, sign-in, session-issuance, inventory, profile-selection, rotation, and revocation source only on the exact requested-and-bound `127.0.0.1:8420` durable listener. Fallback, alternate-loopback, generic local, integration, wildcard or container forwarding, and remote routers omit C1. The earlier PR #93 `BrowserUser`, local password, development account, custom TOTP, WebAuthn-shaped, backup-code, and fabricated OpenID Connect paths remain superseded.
- The session design uses an opaque random secret, digest-only storage, an exact opaque public session identifier, idle and absolute expiry, rotation, bounded activity updates, Origin and Host checks, strict cross-site request forgery protection, and session-local selection of an existing authorized profile grant.
- The C1 route set stays mounted on the exact direct listener when TrailBase is unavailable so the product can report a safe state. Exchange and new Fasti session issuance require a verified installation receipt and persisted active activation. TrailBase runs as a separate pinned service and never authorizes a Fasti operation without Fasti application authorization.
- The TrailBase development package verifies exact v0.33.5 native archives, executables, OCI platform graphs, and licence text. It keeps the account listener on loopback, keeps the native admin listener separate, does not publish the OCI admin listener, and rejects public admin and unconfigured Record API roots. Initialization rotates the first administrator password in memory and never writes it to a receipt or log. Remote account and OAuth route exposure stays unavailable because v0.33.5 accepts protocol-relative redirect values; conformance records the exact limit without following the redirect.
- Every TrailBase installation has its own administrator credential. Operator custody and recovery belong in an operator-selected password manager or equivalent private installation record. Every person has a distinct TrailBase account. Fasti never copies, stores, logs, receipts, or browser-persists the administrator password. Do not share it across installations or add a Fasti secret store for it.
...
- C1's combined package smoke, packaged WebView cookie behavior, cross-platform listener and process behavior, assistive-technology evidence, final security review, exact-head CI, and merged-tree proof;
