...
function defaultNetworkConfiguration(defaultApiUrl: string): NetworkConfiguration {
  return {
    ...
    outbound_policy: {
      ...
      allow_capabilities: ["*"], deny_capabilities: [],
      allow_hosts: ["*"], deny_hosts: [],
      allow_networks: ["public", "loopback", "private"], deny_networks: [],
    },
  };
}
...
function unavailable(message: string): Error { return new Error(message); }
function normalizedEndpoint(value: string): string { return value.replace(/\/+$/, ""); }
...
export async function fetchIntegrationStatus(endpoint: string): Promise<IntegrationRuntimeStatus[]> {
  ...
  if (!response.ok) throw unavailable(`Fasti integration status returned ${response.status}.`);
  const value = (await response.json()) as Partial<IntegrationStatusResponse>;
  if (!Array.isArray(value.integrations) || !value.integrations.every(isIntegrationStatus)) {
    throw unavailable("Fasti integration status did not match the supported contract.");
  }
  return value.integrations;
}

export function createWebHost(defaultApiUrl: string): WorkbenchHost & IntegrationStatusHost {
  return {
    ...
    async saveNetworkConfiguration(input: SaveNetworkConfigurationRequest): Promise<NetworkConfiguration> {
      ...
      if (typeof localStorage !== "undefined") {
        try { localStorage.setItem(NETWORK_STORAGE_KEY, JSON.stringify(config)); } catch {}
      }
      return config;
    },
    async testEndpointConnection(endpoint: string): Promise<EndpointConnectionStatus> {
      ...
      if (!response.ok) throw unavailable(`The endpoint returned status ${response.status}.`);
    ...
    async listIntegrations(): Promise<IntegrationRuntimeStatus[]> {
      ...
      return fetchIntegrationStatus(configured.connection.service_url.value || defaultApiUrl);
    },
    async providerCredentialStatus(): Promise<ProviderCredentialStatus[]> {
      return PROVIDERS.map((provider) => ({ provider: provider.provider, label: provider.label, configured: false, source: "none", writable: false, docs_url: provider.docs_url }));
    },
    async saveProviderCredential(provider: string): Promise<ProviderCredentialStatus[]> {
      throw unavailable(`${provider} credentials require the trusted native or server host. The browser host never accepts or stores provider secrets.`);
    },
    async deleteProviderCredential(provider: string): Promise<ProviderCredentialStatus[]> {
      throw unavailable(`${provider} credentials are not managed by the browser host.`);
    },
    async searchProvider(provider: string, _query: string): Promise<ProviderSearchCandidate[]> {
      throw unavailable(`${provider} search is not active in the browser host. Provider requests must use the governed native or server host.`);
    },
    clearSearchCache(): void {},
    getSearchCacheSize(): number { return 0; },
    async listRecords(): Promise<RecordSummary[]> {
      throw unavailable("Record listing is not active in the browser host. It requires the trusted native or server host.");
    },
  };
}
