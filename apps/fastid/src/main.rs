...
fn local_api_exposure_addr(
    ...
    let Some(external_bind_ip) = external_bind_ip else {
        return Ok((!uses_remote_health_surface(listen_addr)).then_some(listen_addr));
    };
    anyhow::ensure!(
...
async fn main() -> Result<()> {
    ...
    let remote_health_only = local_api_addr.is_none();
    ...
    let app = match configured_data_root {
        Some(data_root) => {
            ...
            api_router(
                kernel,
                local_api_addr.expect("configured durable routes require local exposure"),
                &data_root,
            )
        }
        None => {
