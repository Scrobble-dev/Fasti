...
- **Historical UI reference**: This imported note predates the current Workbench contract. Current navigation uses the Tabler vertical-navbar/offcanvas structure in `brand/DESIGN.md`, not a Ryot-style fixed or mobile rail. The existing real actions, Atkinson typography, theme choices, and no-player boundary remain preserved.
