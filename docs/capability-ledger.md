...
| `observation.accept`                                                             | Durable production HTTP route on loopback (`POST /api/v1/observations`), bearer-authenticated                   |
...
| Identity records, identifiers, and namespaces                                    | Durable production HTTP routes on loopback (`/api/v1/records`, `/api/v1/records/identifiers`, `/api/v1/namespaces`), bearer-authenticated, covered by `cargo xtask contract verify`     |
