...
| Local service status                                                                                | `/status` uses the generated TypeScript health parser in the browser and the shared Rust health contract through trusted Tauri IPC. The packaged diagnostic remains available before setup. | `system.health`; no new capability required.                                                                                                             |
| Browser record list                                                                                 | `GET /api/v1/records` uses a real `identity_read` bearer held only in tab memory. The dialog names its destination and requires confirmation for a non-loopback origin. Reload clears it. The summary is limited to 500 Records and has no cursor. | Existing `identity.record.list` registry/OpenAPI/SDK surface.                                                                                            |
