...
`cargo xtask contract verify --locked` is the deterministic software gate. Its success receipt does not close B1, PR A, or C1. Closure requires evidence bound to the exact reviewed head, including the applicable headless, developer-experience, Tauri, and same-workflow-attempt x86_64/aarch64 low-hardware packages. Until `cargo xtask test milestone --body B1` passes, B1 remains in progress and B2 is not authorized. `cargo xtask test milestone --body C1` now writes a gate-suite receipt only. C1 package smoke, closure manifest, packaged WebView, platform, assistive-technology, review, exact-head, and merged-tree evidence remain pending.
