...
| `MAL_API` | `app.providers.mal.search`, `.anime`, `.manga`; `app.discover.provider_candidates._mal_ranking_candidates.fetcher`; `integrations.imports.mal.MyAnimeListImporter._get_whole_response` | MAL client-ID header for metadata, Discover, and imports |
