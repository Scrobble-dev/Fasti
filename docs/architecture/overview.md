...
The native daemon and CLI are the current executable product shapes. OCI wraps the same binaries and does not add a web build or hidden static fallback. The native daemon and loopback-only container launcher mount bootstrap plus authenticated durable routes when an explicit data root is present. The non-loopback daemon mounts only the authenticated subset behind explicit trusted HTTPS proxy configuration. `apps/web` is the private, unpackaged pre-production Workbench over generated contracts. `apps/desktop` remains a trusted-host review candidate, not a supported package. There is no supported installation, release, player, or deployed web application.
