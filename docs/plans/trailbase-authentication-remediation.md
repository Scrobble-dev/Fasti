...
Apply Kathy Sierra's *Badass: Making Users Awesome*. Success is the person's ability to complete and recover from a real task, not a feature count.
...
Gate `A-POLICY` sets browser values before PR A code. Gate `E-POLICY` sets OAuth values before PR E code. Use current source, RFC requirements, threat model, journeys, and measured operations. Mark missing evidence `BLOCKED`.
...
- approved `SessionPolicy`, idle and absolute expiry;
