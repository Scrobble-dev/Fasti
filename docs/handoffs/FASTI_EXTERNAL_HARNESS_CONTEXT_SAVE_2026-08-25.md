...
- **External audit reports** (not stored in this repository):
  - `fasti_qa_audit.md` — PR gate and mutation test receipts.
  - `fasti_design_review.md` — Visual design, Nielsen heuristics, and WCAG AAA matrix.
  - `fasti_claude_outside_voice_review.md` — Independent outside-voice review.
- **External architecture blueprints** (not stored in this repository):
  - `fasti_ecosystem_comparative_analysis.md` — Ecosystem feature matrix against Floppy, Yamtrack, Scrob, Ryot, Showly.
  - `fasti_batteries_included_goldilocks_plan.md` — Custom types, WebAuthn, PATs, lossless import specs.
- **Component Suite**:
  ...
  - [`packages/ui/src/library-view.svelte`](../../packages/ui/src/library-view.svelte) — Media catalogue with search & filters.
  - [`packages/ui/src/media-detail-view.svelte`](../../packages/ui/src/media-detail-view.svelte) — Media details, identity claims, episode checklists.
  - [`packages/ui/src/reconciliation-view.svelte`](../../packages/ui/src/reconciliation-view.svelte) — Review Inbox & candidate diffs.
  - [`packages/ui/src/settings-view.svelte`](../../packages/ui/src/settings-view.svelte) — Custom fields, importers, PATs.
